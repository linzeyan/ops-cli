...
qrcode.png`, CommandQrcode, CommandRead),
	...
	run := func(cmd *cobra.Command, args []string) {
		...
		case CommandOTP:
			if flags.otpSecret == "" {
				logger.Info(common.ErrInvalidArg.Error(), common.NewField("secret", flags.otpSecret))
				return
			}
			switch flags.otpDigits {
			case 6, 7, 8:
			default:
				logger.Info(common.ErrInvalidArg.Error(), common.NewField("digits", flags.otpDigits))
				return
			}
			switch flags.otpPeriod {
			case 15, 30, 60:
			default:
				logger.Info(common.ErrInvalidArg.Error(), common.NewField("period", flags.otpPeriod))
				return
			}
			...
			default:
				logger.Info(common.ErrInvalidArg.Error(), common.NewField("algorithm", flags.otpAlgorithm))
				return
			}
		...
		case CommandWiFi:
			if flags.wifiSsid == "" {
				logger.Info(common.ErrInvalidArg.Error(), common.NewField("ssid", flags.wifiSsid))
				return
			}
		...
		err := common.GenerateQRCode(flags.text, flags.size, flags.output)
		if err != nil {
			logger.Info(err.Error())
		}
	}
